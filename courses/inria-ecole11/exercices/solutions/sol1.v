...
Compute (even 2, even 3, even 7, even 24).
...
(* 5/ Define a function that takes two arguments.  the first argument
   ...
   by applying f to the result of applying f on p.   Call this function iter2p
*)
...
Definition sp (p : nat * nat) :=
  match p with (a,b) => ex1 a b end.
...
Definition sm a b := if leb b a then a + 1 else b.
...
Definition ex11 (f : nat -> nat -> nat) (p : nat*nat) :=
  match p with (a, b) => (a + 1, f a b) end.
...
Definition ex12 (g : nat -> nat) (f : nat -> nat -> nat) (p : nat * nat) :=
  match p with (a, b) => (g a, f a b) end.
